...
Phone extraction is validated before merge decisions are made. The extractor
removes filename/link noise and rejects numbered-list fragments that can look
like phone numbers. Valid phone evidence, including page-text evidence, then
uses the configured confidence thresholds when deciding whether to keep, add,
replace, or review a value.
